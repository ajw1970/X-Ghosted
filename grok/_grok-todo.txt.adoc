...
* [ ] Update Modal to look and feel like Panel. Still needs contrast improvements in Dim and dark themes. The textbox hint is unreadable.
* [ ] Add gap between eyeballs and link in the panel list
* [ ] Add Identified Posts (count) to header too.
* [ ] Need tp be able to pin/unpin the panel or maybe dock it. Have this be a configurable option that defaults to pinned with a persisted state value.
* [ ] Stop polling when a first order problem is identified.
* [ ] Use build-xGhosted to replace placeholder in xGhosted.template.js with package.json version. Have this included in the Splash Screen.
