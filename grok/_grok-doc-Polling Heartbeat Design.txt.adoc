...
| `false` | `false` | `pollInterval` (2000ms) | - Check URL (`checkUrlDebounced`)  
  - If URL changed:  
...
| `false` | `true` | `pollInterval` (2000ms) | - Same as above (`false`, `false`)  
...
| `true` | `false` | `pollInterval` (2000ms) | - Check URL (`checkUrlDebounced`)  
  - If URL changed: Same as above  
  - Count `cellInnerDiv` posts  
  - Process posts (`highlightPostsDebounced`, delayed 2000ms):  
...
| `true` | `true` | `scrollInterval` (800ms) | - Check URL (`checkUrlDebounced`)  
  ...
  - 2000ms `highlightPosts` delay mitigates duplicates (e.g., 14:49:56.508).  
...
- **Continuous Polling**: Always `pollInterval` (2000ms) for URL changes and processing (tested April 27, 2025, 15:10). Fixes panel clearing bug.
...
- **Duplicates**: 2000ms delay and debouncing for `highlightPosts`. Unit tests for `highlightPosts`, DOM mutations, scrolling planned post-refactor (April 27, 2025, 13:35).
