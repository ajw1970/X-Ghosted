function Panel({ state, config, currentMode, linkPrefix, toggleThemeMode, onCopyLinks, startDrag, onEyeballClick, flagged, totalPosts, isScanning, isScrolling, userProfileName, onToggleVisibility, onToggleTools, onToggleScanning, onToggleAutoScrolling, onExportCsv, onOpenModal, onCloseModal, onSubmitCsv, onClearPosts, onOpenAbout, onToggleDropdown, }) {
    const themeOptions = ['dark', 'dim', 'light'].filter((option) => option !== currentMode);
    return window.preact.h('div', null, window.preact.h('div', {
        id: 'xghosted-panel',
        style: {
        ...
        ? window.preact.h(window.preact.Fragment, null, window.preact.h('div', { className: 'toolbar' }, window.preact.h('button', {
        ...
        }, window.preact.h('div', { className: 'custom-dropdown' }, window.preact.h('button', {
            className: 'panel-button dropdown-button',
            onClick: onToggleDropdown,
            'aria-expanded': state.isDropdownOpen,
            'aria-label': 'Select Theme',
        }, currentMode.charAt(0).toUpperCase() +
            currentMode.slice(1), window.preact.h('i', {
            className: state.isDropdownOpen
                ? 'fas fa-chevron-up'
                : 'fas fa-chevron-down',
            style: { marginLeft: '8px' },
        })), state.isDropdownOpen &&
            window.preact.h('div', { className: 'dropdown-menu' }, themeOptions.map((option) => window.preact.h('div', {
                key: option,
            ...
            }, option.charAt(0).toUpperCase() + option.slice(1)))))), window.preact.h('div', {
            style: {
                display: 'flex',
                flexDirection: 'column',
                gap: '12px',
            ...
            style: { marginRight: '8px' },
        }), 'Export CSV'), window.preact.h('button', {
            className: 'panel-button',
            onClick: onOpenModal,
            ...
            style: { marginRight: '8px' },
        }), 'Import CSV'), window.preact.h('button', {
            className: 'panel-button',
            ...
            style: { marginRight: "8px" },
        }), "Export Metrics"), window.preact.h('button', {
            className: 'panel-button',
            onClick: onOpenAbout,
        ...
        }, window.preact.h('span', { className: 'header-text-group' }, 'Processed Posts (', totalPosts, ') Concerns (', flagged.length, '):', window.preact.h('span', {
            style: {
                cursor: 'pointer',
                fontSize: '14px',
        ...
        }, window.preact.h('i', { className: 'fas fa-copy' }))), window.preact.h('span', {
        ...
        }))), window.preact.h('div', { className: 'problem-links-wrapper' }, flagged.length > 0
            ? flagged.map(([href, { analysis }]) => window.preact.h('div', { className: 'link-row', key: href }, analysis.quality.name === 'Potential Problem'
                ...
                : window.preact.h('span', {
                    ...
                    'aria-label': analysis.quality.name === 'Problem'
                        ? 'Problem post'
                        : 'Problem adjacent post'
                }), window.preact.h('span', { className: 'link-item' }, window.preact.h('a', {
