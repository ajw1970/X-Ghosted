function SplashPanel(doc, logger, version, userProfileName, pollInterval, scrollInterval) {
    ...
    this.startDrag = function (e) {
        if (e.target.tagName === 'BUTTON') return; // Prevent dragging when clicking the close button
        e.preventDefault();
        ...
        this.container.style.transform = 'none'; // Remove transform for direct positioning
        this.container.style.top = `${this.initialTop}px`;
        this.container.style.left = `${this.initialLeft}px`;
        this.document.addEventListener('mousemove', (e) => this.onDrag(e));
        this.document.addEventListener('mouseup', () => this.stopDrag(), { once: true });
    ...
    this.render = function (config) {
        ...
        closeButton.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent mousedown from triggering drag
